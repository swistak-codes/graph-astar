 ...
 *
 * MODIFIED TO COUNT ITERATIONS
 */

import { Node } from "./node";
import { NodeListIterator } from "./node-list-iterator";
import {
  FibonacciHeap as FibonacciHeapApi,
  CompareFunction,
  INode,
} from "./types";

type NodeWithIterations<K, V> = { node: Node<K, V> | null; iterations: number };

export class FibonacciHeap<K, V> implements FibonacciHeapApi<K, V> {
  private _minNode: Node<K, V> | null = null;
  private _nodeCount = 0;
  private readonly _compare: CompareFunction<K, V>;
  ...
  public extractMinimum(): Node<K, V> | null {
    return this.extractMinimumWithIterationCount().node;
  }

  public extractMinimumWithIterationCount(): NodeWithIterations<K, V> {
    let iterations = 0;
    ...
    if (extractedMin) {
      ...
      if (this._minNode) {
        const consolidateResult = this._consolidate(this._minNode);
        this._minNode = consolidateResult.node;
        iterations += consolidateResult.iterations;
      }
    }
    return {
      node: extractedMin,
   ...
   */
  private _consolidate(minNode: Node<K, V>): NodeWithIterations<K, V> {
    let iterations = 0;
    const aux = [];
    ...
    return { node: newMinNode, iterations };
  }
